...
[tokenGeneratorSamples]: #TokenGeneratorSamples "Token Generator Samples"
...
[xamarinQSG]:            #XamarinSDK            "Xamarin Quick Start Guide"
[electronQSG]:           #ElectronSDK           "Electron Quick Start Guide"
...
[DockerOverview]: https://www.docker.com/what-docker
[Ubuntu1404]: https://www.docker.com/docker-ubuntu
[DockerEngine]: https://docs.docker.com/engine/installation/linux/ubuntu/#install-from-a-package
[DockerDocSet]: https://docs.docker.com/
[TokenGeneration]: https://support.vidyo.io/hc/en-us/articles/115003127747-How-to-generate-a-token
[DockerLinuxInstall]: https://docs.docker.com/engine/installation/linux/ubuntu/
...
Vidyo.io&trade; is a platform as a service (PaaS) that enables developers to provide high-quality, real-time video communication capabilities.  The service includes the VidyoClient SDK, which provides the APIs for integrating such communication capabilities into a variety of applications and workflows.
...
To dig deeper into Vidyo.io, we suggest you check out its [Key Concepts][keyConcepts].
...
We are here to make sure you get the best video experience your customers can ever wish for. Just email us at <a href="mailto:support@vidyo.io" target="_top">support@vidyo.io</a>.
...
1. **VideoClient SDK** - This is the client-side SDK that your application calls when it needs to add video support. The VidyoClient SDK shares the same APIs across the multiple [platforms][supportedPlatforms].
...
Check out more about [tokens][tokens] and [resources][resources].
...
public class MyClass : Connector.IConnect
{
    ...
    private void ConnectToResource(IntPtr viewHandle, uint viewWidth, uint viewHeight, string host, string token, string displayName, string resourceId) {
...
<a class="headerAnchor" name="Tokens"></a>
## Tokens
...
For each application, you must create a **DeveloperKey** and **ApplicationID** in the [API Key](https://developer.vidyo.io/dashboard#tab_apikey) section of the Vidyo.io site and securely store it in your application backend.
Then, each time you need to grant access to the Vidyo.io service, you must generate a **Token** from the **DeveloperKey** and **ApplicationID** as described below and pass it to their endpoint application or the **VidyoConnector** application.

![](./images/overviewToken.jpg)
...
#### User Name
A token is associated to a specific user in Vidyo.io. To that end, when a token is generated, a user name needs to be provided.
The user name is an open-ended alphanumeric string ([a-z,A-Z,0-9]) of your choosing which will be provisioned on the fly.

> Vidyo.io will refer to users in Participant and Chat related APIs as user@applicationId.  
> Using special characters within the user name will result in an invalid token.
...
<a class="headerAnchor" name="TokenGeneratorSamples"></a>
#### Samples

Python sample generator: https://static.vidyo.io/latest/utils/generateToken.py
```bash
python3 generateToken.py --key=rUlaMASgt1Byi4Kp3sKYDeQzo --appID=ApplicationID --userName=user1 --expiresInSecs=10000
```

 Java sample generator: https://static.vidyo.io/latest/utils/generateToken.jar
```bash
java -jar generateToken.jar --key=rUlaMASgt1Byi4Kp3sKYDeQzo --appID=ApplicationID --userName=user1 --expiresInSecs=10000
```

Node.js sample generator: https://static.vidyo.io/latest/utils/generateToken.js
```bash
...
<a class="headerAnchor" name="Resources"></a>
## Resources
...
VidyoParticipant represents each user that is connected to the same resourceId (that is, connected to the same video conference/session). Once you are connected, **RegisterParticipantEventListener** can be triggered to receive notifications upon participant updates.
...
Windows and monitors can be shared in a session taking place on a resourceId. Passing *null* into **SelectLocalWindowShare / SelectLocalMonitor** will stop the sharing.
...
<a class="headerAnchor" name="LiveDemo"></a>
## Live Demo

You can use the VidyoConnector helper application to easily set up a live demo. The [Demo](https://developer.vidyo.io/demo) section can launch the VidyoConnector sample as well as generate a temporary [token][tokens].

> A separate demo link must be generated for each participant.

Follow these instructions to launch the live demo:

1. [Sign up](https://developer.vidyo.io/signup).
1. Navigate to the [Demo](https://developer.vidyo.io/demo) section.
1. To launch the VidyoConnector Web app with an auto-generated token, click **LAUNCH**.
   * You can generate a temporary link and email/send it to other demo participants by clicking **GENERATE LINK**.
1. The VidyoConnector app will launch in one of the following ways:
   ...
   * As a protocol handler link on iOS and Android. After the VidyoConnector is built and installed, follow the [iOS Quick Start][iOSQSG] or [Android Quick Start][androidQSG] instructions to build the necessary clients.

> Click here to find the sample [token generator][tokenGeneratorSamples] script.
...
| token             |                | Yes      | An access [token][tokens] required to authenticate the session. A new token should be created for every endpoint.      |
...
| hideConfig        | 0              | No       | **0** = when in preview mode (not connected to a resource), the video conference connection settings (host, token, displayName, resourceId) are displayed and may be configured. <br>**1** = the video conference connection settings are never displayed and thus must have been passed to the app via command line or URL<br>**Note:** On Android, use "false" and "true", rather than "0" and "1". |
| autoJoin          | 0              | No       | **0** = upon starting the application, do not automatically connect to the resource. <br>**1** = upon starting the application, automatically attempt to connect to the resource. <br>**Note:** On Android, use "false" and "true", rather than "0" and "1". |
...
* [macOS][macOSPluginInstaller]
* [Windows 32bit][windowsPluginInstaller32bit]
* [Windows 64bit][windowsPluginInstaller64bit]

#### Invocation
```bash
https://static.vidyo.io/latest/connector/VidyoConnector.html?host=prod.vidyo.io&token=SomeToken&resourceId=SomeRoom
```
...
#### Mac Invocation
```bash
open VidyoConnector.app --args -host prod.vidyo.io -token SomeToken -resourceId SomeRoom -hideConfig 1 -autoJoin 1 -allowReconnect 0
```

#### Windows Invocation
```bash
VidyoConnector.exe -host prod.vidyo.io -token SomeToken -resourceId SomeRoom -hideConfig 0
```
...
```
[[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"VidyoConnector://?token=someToken&resourceId=SomeRoom&hideConfig=0&returnURL=URLSchemeOfLaunchingApp://"]];
```
...
Please refer to the platform section in the [Overview][supportedPlatforms] for links to download the VidyoClient SDK and a quick-start instructions for building the VidyoConnector sample.
...
| WebRTC        | yes    | yes     | yes<sup>2</sup> | no              | yes<sup>2<sup> |
| Native WebRTC | yes    | no      | no              | no              | no             |
...
1. Within this folder, locate the `samples/VidyoConnector/js/VidyoConnector.html` file.
1. Open `VidyoConnector.html` in a supported browser.
...
1. The browser displays the Vidyo.io app with a login form on the left and a preview image from the video camera on the right.
1. Review the Vidyo.io app login form. See [VidyoConnector parameters][vidyoConnector] for more details.
...
1. Extract the downloaded `.zip` file to a folder.
1. Use the IDE to open the `VidyoConnector12.sln`, which is located in the extracted folder.
...
1. Extract the downloaded `.zip` file to a folder.
1. Use Xcode to open `VidyoConnector.xcodeproj`, which is located in the extracted folder.
...
1. Extract the downloaded `.zip` file to a folder.
1. Use Xcode to open `VidyoConnector-iOS.xcodeproj`, which is located in the extracted folder.
...
1. Extract the downloaded `.zip` file to a folder.
1. In Android Studio, use **Import project** to open and import the Android project for the app, which is located in the extracted folder (select the  `samples/VidyoConnector/android` folder to import).
...
<a class="headerAnchor" name="XamarinSDK"></a>
## Xamarin

This guide shows how to use Xamarin to quickly download, build, and run the VidyoConnector application on an iOS or Android device and make alterations to it.

### Run VidyoConnector from GitHub

This section explains how to download and run the Xamarin Forms VidyoConnector application.

#### Prerequisites

* [Visual Studio IDE](https://www.xamarin.com/visual-studio)

#### Download and Run the App

1. Download the [VidyoConnector-xamarin app](https://github.com/Vidyo/VidyoConnector-xamarin) from GitHub.
1. Download the [iOS SDK][supportedPlatforms] package.
1. Extract the downloaded `.zip` file to a folder.
1. Move the unzipped VidyoClient-iOSSDK folder to the VidyoConnector-xamarin directory.
1. Download the [Android SDK][supportedPlatforms] package.
1. Extract the downloaded `.zip` file to a folder.
1. Move the unzipped VidyoClient-AndroidSDK folder to the VidyoConnector-xamarin directory.
1. Use Visual Studio to open `VidyoConnector.sln`, located in the extracted folder.
1. In Visual Studio, set VidyoConnector.iOS as the Startup Project for iOS devices; use VidyoConnector.Android for Android devices.
1. In Visual Studio, run the app. Visual Studio builds, loads, and runs the app on the iOS or Android device.
1. In the app that appears, enter the required [VidyoConnector parameters][vidyoConnector].
1. Click the Connect icon.

### Code Walkthrough

In Visual Studio, select the Project navigator view and inspect the following files:
1. `VidyoConnector > MainPage.xaml / MainPage.xaml.cs`
1. `VidyoConnector > ViewModel.cs`
1. `VidyoConnector.Android > VidyoController.cs`.

These files contain the primary logic of the VidyoConnector application. To create a custom app, you should modify and extend these files.

These files contain the following logic:

* Initialize and manage session communications
* Manage UI and device events

<a class="headerAnchor" name="ElectronSDK"></a>
## Electron

Please see the [VidyoConnector-electron app](https://github.com/Vidyo/VidyoConnector-electron) project on GitHub for instructions.

<!--Media Bridge-->

---

<a class="headerAnchor" name="MediaBridge"></a>
# Media Bridge
...
<a class="headerAnchor" name="WhatsDocker"></a>
## What's Docker?
...
<a class="headerAnchor" name="MediaBridgePrivacy"></a>
## Privacy
Note that any activity involving recording or monitoring the content of calls, or streaming of calls to others, must comply with any applicable laws, which typically requires, at a minimum, providing notification to all participants of such activity. Please see the <a href="https://www.vidyo.com/cloudservicestos" target="_blank">Vidyo Cloud Terms of Service</a> and <a href="https://www.vidyo.com/cloudservices-user-terms-and-conditions" target="_blank">Vidyo Cloud User Terms and Conditions</a>.

---

<a class="headerAnchor" name="WhatsRecorder"></a>
## What's Media Bridge?
...
<a class="headerAnchor" name="BeforeYouBegin"></a>
## Before You Begin

Before you begin, ensure that your Linux server is [configured with a running Docker Engine][DockerLinuxInstall].

### What you will need

* A Linux server running [Ubuntu 14.04][Ubuntu1404] or newer.
* The [Docker Engine][DockerEngine] package installed and running on your Ubuntu Linux server.
* The complete [Docker Documentation set][DockerDocSet].
* Access tokens to vidyo.io (see [How to generate a Token][TokenGeneration]).
...
<a class="headerAnchor" name="RecorderSetup"></a>
## Set-up
...
2.  Create a recording configuration file (see <a href="#RecordingConfigFile">Recording Configuration File</a>).
...
<a class="headerAnchor" name="StartRecording"></a>
...
Depending on the options set in the config file, the mediabridge will immediately start recording/streaming once you run this command.

---

<a class="headerAnchor" name="MonitorRecording"></a>
## Monitor a Recording or Live Stream
...
<a class="headerAnchor" name="StopRecording"></a>
...
<a class="headerAnchor" name="RetrieveRecording"></a>
...
To override this, use the <a href="#destination">**destination**</a> option described in the **Recording Configuration File Options** table below.

---

<a class="headerAnchor" name="SIPGatewayCalls"></a>
...
```
docker run -d --name sipCall -p 50000-50100:50000-50100/udp -p 5060:5060/udp vidyo/mediabridge /sbin/my_init
```
...
<a class-"headerAnchor" name="ConvertFLV2MP4"></a>
## Convert FLV Files to MP4 Format

You can use FFMPEG to convert an FLV file to MP4. The FFMPEG conversion command is `ffmpeg -i <flv file name> -c:v copy -c:a aac -strict -2 <mp4 file name>`. You can download FFMPEG at: `https://ffmpeg.org/https://ffmpeg.org/`

---

<a class-"headerAnchor" name="InstallCustCertAuth"></a>
...
<a class="headerAnchor" name="RecorderTroubleshooting"></a>
## Troubleshooting
...
<a class="headerAnchor" name="ResourceRequirements"></a>
...
<a class="headerAnchor" name="RecordingConfigFile"></a>
...
| **videoCodec**                  | You can set the video codec to the following lossy compression standards: `H264`, `H263`, `VP8`, `VP9`. The default setting is `H264`. **Note**: FLV format only supports H264 and H263 video codecs. Recordings configured for unsupported codecs fail to start.|
| **audioCodec**                  | You can set the audio codec to the following lossy compression standards: `AAC` (32K kHZ), `PCM` (11.025K kHZ), `OPUS` (48K kHZ), `SILK8` (8K kHz), `SILK16` (16K kHz), `SPEEX` (32K kHz). The default setting is `AAC`. **Note**: FLV format only supports AAC and PCM audio codecs. Recordings configured for unsupported codecs fail to start.|
...
| **maxCallDuration**             | The maximum duration in minutes. After this time duration the mediabridge will automatically stop. |
...
| **samplingRate**                | The number of samples of audio carried per second. |

---

<a class="headerAnchor" name="SampleConfigFile"></a>
...
#Debug settings
log=warn          # options - full|debug|info|warn|error|none
```
